...
function RuntimeCommunication(transport, graph) {
    var self = this;
    ...
    var onDataReceived = function(da) {
        ...
        for (var startIdx=0; startIdx < Math.floor(offset/cmdSize)*cmdSize; startIdx+=cmdSize) {
            ...
            var receiveHandler = function() {
                self.emit("reponse", arguments);
            }
            parseReceivedCmd(b, graph, receiveHandler);
        }
    ...
    self.transport.on("data", onDataReceived);

    // Input/Output
    this.sendCommandBuffer = function(data, callback) {
        ...
        setTimeout(function() {
            ...
            var sendCmd = function(dataBuf, index) {
                self.transport.write(dataBuf.slice(index, index+cmdSize), function() {
                    //console.log("wrote", dataBuf.slice(index, index+cmdSize));
                    //console.log("wrote graph");
                    if (index < dataBuf.length) {
                        if (self.transport.getTransportType() === "HostJavaScript") {
...
util.inherits(RuntimeCommunication, events.EventEmitter);


var uploadGraph = function(communication, data, graph, receiveHandler) {

    // FIXME: will be attached multiple times
    communication.on("response", function() {
        receiveHandler(arguments);
    });
    ...
    communication.sendCommandBuffer(data, function () {
...
var handleMessage = function (message, wsConnection, graph, getTransport, debugLevel) {
  if (message.type == 'utf8') {
    ...
    var sendFunc = function(response) { wsConnection.sendUTF(JSON.stringify(response)) };
    var connection = { send: sendFunc };
...
var registerRuntime = function(rtinfo, regoptions) {
    ...
    rt.register(function (err, ok) {
        if (err) {
            console.log('Info: Could not register with Flowhub', err);
        } else {
