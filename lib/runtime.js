...
// Create a class to handle communication with device
// should take Transport as input
// commands sent in order, 1 at a time
// pendingCommand = commandId
// map between command and response
// if response is wrong, fail
// if response is correct, emit result and continue sending
// should we do queueing in this class? would be convenient
// TODO: all commands must have a response, and it sent by runtime
// easy way to upload a whole graph/stream and only get result when whole thing done/error
// should also handle that when device is reset, it sends us its state
// can we detect the reset on serialport transport so we know what to expect?
